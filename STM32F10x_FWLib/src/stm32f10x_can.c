...
/**
 * @brief 将 CAN 外设寄存器取消初始化为其默认复位值。
 * @param CANx：其中 x 可以是 1 或 2 以选择 CAN 外设。
 * @retval 无。
 */
 ...
 * @brief 根据指定初始化CAN外设
 * CAN_InitStruct 中的参数。
 * @param CANx: 其中 x 可以是 1 或 2 来选择 CAN
 * 外围设备。
 * @param CAN_InitStruct：指向 CAN_InitTypeDef 结构的指针
 * 包含配置信息
 * CAN 外围设备。
 * @retval 常量表示初始化成功，这将是
...
/**
 * @brief 根据指定初始化CAN外设
 * CAN_FilterInitStruct 中的参数。
 * @param CAN_FilterInitStruct: 指向 CAN_FilterInitTypeDef 的指针
 * 包含配置的结构信息。
 * @retval 无。
 */
...
/**
 * @brief 用默认值填充每个 CAN_InitStruct 成员。
 * @param CAN_InitStruct：指向 CAN_InitTypeDef 结构的指针将被初始化。
 * @retval 无。
 */
...
/**
 * @brief 选择从 CAN 的起始组过滤器。
 * @note 此功能仅适用于 STM32 Connectivity 线路设备。
 * @param CAN_BankNumber：从 1..27 中选择起始从库过滤器。
 * @retval 无。
 */
...
/**
 * @brief 启用或禁用 CAN 的 DBG 冻结。
 * @param CANx：其中 x 可以是 1 或 2 以选择 CAN 外设。
 * @param NewState：CAN 外设的新状态。 这个参数可以
 * 是：启用或禁用。
 * @retval 无。
 */
...
/**
 * @brief 启用或禁用 CAN Time TriggerOperation 通信模式。
 * @param CANx：其中 x 可以是 1 或 2 以选择 CAN 外设。
 * @param NewState : 模式新状态，可以是@ref FunctionalState 之一。
 * @note 启用时，时间戳 (TIME[15:0]) 值在最后发送
 * 8 字节消息的两个数据字节：数据字节 6 中的 TIME[7:0]
 * 和数据字节 7 中的 TIME[15:8]
 * @note DLC 必须编程为 8 才能使时间戳（2 字节）成为
 * 通过 CAN 总线发送。
 * @retval 无
 */
 ...
 * @brief 启动消息的传输。
 * @param CANx：其中 x 可以是 1 或 2 以选择 CAN 外设。
 * @param TxMessage：指向包含 CAN Id、CAN 的结构的指针
 * DLC 和 CAN 数据。
 * @retval 用于传输的邮箱号
 * 如果没有空邮箱，则为 CAN_TxStatus_NoMailBox。
 */
...
/**
 * @brief 检查消息的传输。
 * @param CANx: 其中 x 可以是 1 或 2 来选择
 * CAN 外围设备。
 * @param TransmitMailbox：用于的邮箱号码
 *                          传播。
 * @retval CAN_TxStatus_Ok 如果 CAN 驱动程序发送消息 CAN_TxStatus_Failed
 * 在其他情况下。
 */
 ...
 * @param RxMessage：指向结构接收消息的指针，其中包含
 * CAN Id、CAN DLC、CAN 数据和 FMI 编号。
 * @retval 无。
 */
...
/**
   * @brief 选择 CAN 操作模式。
   * @param CAN_OperatingMode：CAN 操作模式。 这个参数可以是一个
   * @ref CAN_OperatingMode_TypeDef 枚举。
   * @retval 请求模式的状态，可以是
 ...
 * @brief 进入低功耗模式。
 * @param CANx：其中 x 可以是 1 或 2 以选择 CAN 外设。
 * @retval status: CAN_Sleep_Ok 如果进入睡眠状态，CAN_Sleep_Failed
 * 其他情况。
 */
 ...
 * @brief 唤醒 CAN。
 * @param CANx：其中 x 可以是 1 或 2 以选择 CAN 外设。
 * @retval status: CAN_WakeUp_Ok 如果离开睡眠模式，CAN_WakeUp_Failed 在一个
 * 其他情况。
 */
...
/**
 * @brief 返回 CANx 的最后一个错误代码 (LEC)。
 * @param CANx：其中 x 可以是 1 或 2 以选择 CAN 外设。
 * @retval CAN_ErrorCode：指定错误代码：
 * - CAN_ERRORCODE_NoErr 无错误
 * - CAN_ERRORCODE_StuffErr 资料错误
 ...
 * @note 如果在接收过程中发生错误，此计数器会递增
 * 1 或 8 取决于 CAN 定义的错误条件
 *         标准。 每次成功接收后，计数器
 * 如果其值高于 128，则减 1 或重置为 120。
